...
export default function DenseTable({ clientsList, query, orderClients }) {
	const {
		navigate,
		setCurrentClient,
		showModal,
	...
	const openModal = (client) => {
		...
		localFormCharge.name = client.nome;
			...
			>
				<TableHead>
					<TableRow>
						<TableCell
							align='left'
							className='first-headline headlines'
						>
							<img
								alt='Botão ordernar'
								src={orderIcon}
					...
					{handleGetClients() &&
						handleGetClients().map((row) => (
							...
							>
								...
								>
									<Link fontWeight='700'>{row.nome}</Link>
								</TableCell>
								...
								<TableCell
									className={
										!row.status ? "info-red" : "info-green"
									}
									align='left'
								>
