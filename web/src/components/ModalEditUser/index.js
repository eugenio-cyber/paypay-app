...
function lengthStringPassword(string) {
...
export default function ModalEditUser({ getUser }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (forms.password) {
      if (lengthStringPassword(forms.password)) {
        localWarning.active = true;
        localWarning.message = "A senha precisa ter no mínimo 6 digitos";
    ...
    if (forms.cpf && lengthString(forms.cpf)) {
      localWarning.active = true;
      localWarning.message = "O CPF informado não é válido, ex: 99999999999";
    ...
    if (forms.phone && (forms.phone.length < 11 || forms.phone.length > 16)) {
      localWarning.active = true;
      localWarning.message = "O telefone informado é inválido, ex: 99999999999";
    ...
    if (forms.name && forms.name !== user.name) {
      data.name = forms.name;
    }

    if (forms.cpf && forms.cpf !== user.cpf) {
      data.cpf = forms.cpf;
    }

    if (forms.email && forms.phone !== user.email) {
      data.email = forms.email;
    }

    if (forms.phone && forms.phone !== user.phone) {
      data.phone = forms.phone;
    }

    if (forms.password) {
      data.password = forms.password;
    }
