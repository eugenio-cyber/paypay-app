...
const panelCharges = async (_, res) => {
	try {
		// const allCharges = await queryBuilder("vw_billings")
		//.count("* as total");

		const paidTotal = await queryBuilder("vw_billings")
			.where({ ch_status: "Paga" })
			.sum("valor as total");

		const pendingTotal = await queryBuilder("vw_billings")
			.where({ ch_status: "Pendente" })
			.sum("valor as total");

		const overdueTotal = await queryBuilder("vw_billings")
			.where({ ch_status: "Vencida" })
			.sum("valor as total");

		const paidCount = await queryBuilder("vw_billings")
			.where({ ch_status: "Paga" })
			.count("* as total");

		const pendingCount = await queryBuilder("vw_billings")
			.where({ ch_status: "Pendente" })
			.count("* as total");

		const overdueCount = await queryBuilder("vw_billings")
			.where({ ch_status: "Vencida" })
			.count("* as total");

		const paidList = await queryBuilder("vw_billings")
			.where({ ch_status: "Paga" })
			.select("*")
			.limit(4);

		const pendingList = await queryBuilder("vw_billings")
			.where({ ch_status: "Pendente" })
			.select("*")
			.limit(4);

		const overdueList = await queryBuilder("vw_billings")
			.where({ ch_status: "Vencida" })
			.select("*")
			.limit(4);
...
const updateCharge = async (req, res) => {
	...
	try {
		const chargeExists = await queryBuilder("charges")
			.where({ id })
			.first();
...
const deleteCharge = async (req, res) => {
	...
	try {
		...
		if (charge.status !== 'Pendente') {
			return messages(res, 400, "Uma cobrança só pode ser excluída se o status for Pendente");
		}
