...
const registerClient = async (req, res) => {
	...
	try {
		const client = await queryBuilder("clients")
			.insert(data)
			.returning("*");
		if (!client) {
...
const panelClients = async (_, res) => {
	try {
		const defaultingsList = await queryBuilder("vw_billings")
			.select("nome", "ch_id", "valor", "vencimento", "cl_status")
			.where({ cl_status: false })
			.orderBy("ch_id", "desc")
			.groupBy("nome", "ch_id", "valor", "vencimento", "cl_status")
			.limit(4);

		const dealList = await queryBuilder("vw_billings")
			.select("nome", "ch_id", "valor", "vencimento", "cl_status")
		...
		const defaultingsNumber = await queryBuilder("vw_billings")
			.where({ cl_status: false })
			.count("cl_id as total");

		const dealNumber = await queryBuilder("vw_billings")
			.where({ cl_status: true })
			.count("cl_id as total");
